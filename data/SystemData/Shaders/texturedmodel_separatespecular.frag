...
// PBR-lite controls (see common/pbr.glsl). In the PBR path there is no need for
// the separate-specular trick (Cook-Torrance specular is energy-correct in one
// pass), so this shader behaves identically to texturedmodel.frag under PBR.
...
void main()
{
    ...
    else
    {
        ...
        FRAGCOLOR += processLight(fragColour, normal, u_lightDir[4], u_lightDiffuseColour[4], u_lightSpecularColour[4], u_lightAmbientColour[4], u_specularAmount, u_specularExponent);
