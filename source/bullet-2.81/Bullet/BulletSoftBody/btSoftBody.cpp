...
void	btSoftBody::initDefaults()
{
	...
	m_cfg.timescale		=	1;
  m_cfg.ropeNodeArea = btScalar(0.0);
	m_cfg.viterations	=	0;
	m_cfg.piterations	=	1;	
	m_cfg.diterations	=	0;
	m_cfg.citerations	=	4;
	...
	if( n.m_im>0 )
	{
		...
		{				
			...
			if(rel_v2>SIMD_EPSILON)
			{
				...
				else if (m_cfg.aeromodel == btSoftBody::eAeroModel::V_Drag)
				{
          btVector3 dragDir = -rel_v/rel_v_len;
					/* Compute forces	*/ 
          btScalar f = 0.5f * medium.m_density * rel_v2 * n.m_area * kDG;
  				btVector3 force = dragDir * f;
					ApplyClampedForce(n, force, dt);
				}	
			}
		}
	...
	if (averageArea)
	{
		...
		for(i=0,ni=m_nodes.size();i<ni;++i)
		{
			if(counts[i]>0)
				...
				m_nodes[i].m_area=m_cfg.ropeNodeArea;
		}
	}
	else
	{
...
void				btSoftBody::PSolve_Anchors(btSoftBody* psb,btScalar kst,btScalar ti)
{
    ...
    a.m_appliedForce += impulse * psb->m_sst.isdt;
...
const char*	btSoftBody::serialize(void* dataBuffer, class btSerializer* serializer) const
{
	...
	sbd->m_config.m_ropeNodeArea = m_cfg.ropeNodeArea;
	sbd->m_config.m_maxVolume = m_cfg.maxvolume;
