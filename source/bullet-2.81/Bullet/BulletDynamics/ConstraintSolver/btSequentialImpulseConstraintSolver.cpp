...
btScalar btSequentialImpulseConstraintSolver::solveGroupCacheFriendlySetup(btCollisionObject** bodies, int numBodies, btPersistentManifold** manifoldPtr, int numManifolds,btTypedConstraint** constraints,int numConstraints,const btContactSolverInfo& infoGlobal,btIDebugDraw* debugDrawer,btStackAlloc* stackAlloc)
{
	...
	for (int i=0;i<numBodies;i++)
	{
		...
		if (body && body->getInvMass())
		{
			...
			btVector3 gyroAngVelChange (0,0,0);
			if (body->getFlags()&BT_ENABLE_GYROSCOPIC_FORCE_EXPLICIT)
			{
				gyroAngVelChange = body->computeGyroscopicForce(infoGlobal.m_maxGyroscopicForce)*body->getInvInertiaTensorWorld()*infoGlobal.m_timeStep;;
			}
			if (body->getFlags()&BT_ENABLE_GYROSCOPIC_FORCE_IMPLICIT_EWERT)
			{
				gyroAngVelChange = body->computeGyroscopicImpulseImplicit_Ewert(infoGlobal.m_timeStep);
			}
			if (body->getFlags()&BT_ENABLE_GYROSCOPIC_FORCE_IMPLICIT_COOPER)
			{
				gyroAngVelChange = body->computeGyroscopicImpulseImplicit_Cooper(infoGlobal.m_timeStep);
			}
			if (body->getFlags()&BT_ENABLE_GYROSCOPIC_FORCE_IMPLICIT_CATTO)
			{
				gyroAngVelChange = body->computeGyroscopicImpulseImplicit_Catto(infoGlobal.m_timeStep);
			}
			btVector3 extraAngularMomentum = body->getWorldTransform().getBasis() * body->getExtraLocalAngularMomentum();
			btVector3 extraGyroTorque = body->getAngularVelocity().cross(extraAngularMomentum);

			solverBody.m_linearVelocity += body->getTotalForce()*body->getInvMass()*infoGlobal.m_timeStep;
      btVector3 angAccel = (body->getTotalTorque()-extraGyroTorque)*body->getInvInertiaTensorWorld();

#define MAX_ANG_ACCEL 1000.0f
      btScalar a = angAccel.length();
      if (a > MAX_ANG_ACCEL)
        angAccel *= MAX_ANG_ACCEL / a;
			solverBody.m_angularVelocity += angAccel*infoGlobal.m_timeStep;

      solverBody.m_angularVelocity += gyroAngVelChange;

#define MAX_ANGVEL SIMD_HALF_PI
      /// clamp angular velocity. collision calculations will fail on higher angular velocities	
      btScalar angvel = solverBody.m_angularVelocity.length();
      if (angvel*infoGlobal.m_timeStep > MAX_ANGVEL)
      {
        solverBody.m_angularVelocity *= (MAX_ANGVEL/infoGlobal.m_timeStep) /angvel;
      }
