...
void	btRigidBody::setupRigidBody(const btRigidBody::btRigidBodyConstructionInfo& constructionInfo)
{
	...
	m_extraLocalAngularMomentum.setValue(btScalar(0.0), btScalar(0.0), btScalar(0.0));
...
btMatrix3x3 btRigidBody::getWorldInertiaTensor() const
{
  btVector3 inertiaLocal = getLocalInertia();
  btMatrix3x3 inertiaTensor = m_worldTransform.getBasis().scaled(inertiaLocal) * m_worldTransform.getBasis().transpose();
...
btVector3 btRigidBody::computeGyroscopicImpulseImplicit_Ewert(btScalar step) const
{
	...
	// one step of newton's method - it converges really quickly
	{
...
void btRigidBody::integrateVelocities(btScalar step) 
{
	...
	btVector3 extraAngularMomentum = m_worldTransform.getBasis() * m_extraLocalAngularMomentum;
	btVector3 gyro = m_angularVelocity.cross(extraAngularMomentum);
	m_linearVelocity += m_totalForce * (m_inverseMass * step);
	m_angularVelocity += m_invInertiaTensorWorld * (m_totalTorque - gyro) * step;
