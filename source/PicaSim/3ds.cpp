...
void CLoad3DS::ProcessNextChunk(ThreeDSModel *pModel, tChunk *pPreviousChunk)
{
    ...
    while (pPreviousChunk->bytesRead < pPreviousChunk->length)
    {
        ...
        case MATERIAL:                     // This holds the material information
            {
                ...
                pModel->mMaterials.push_back(new ThreeDSMaterialInfo);
        ...
        case OBJECT:                    // This holds the name of the object being read
            ...
            pModel->mObjects.push_back(new ThreeDSObject);
            
            // Get the name of the object and store it, then add the read
            // bytes to our byte counter.
            m_CurrentChunk->bytesRead += GetString(pModel->mObjects.back()->strName);
            
            // Now proceed to read in the rest of the object information
            ProcessNextObjectChunk(pModel, pModel->mObjects.back(), m_CurrentChunk);
            break;
...
void CLoad3DS::ProcessNextMaterialChunk(ThreeDSModel *pModel, tChunk *pPreviousChunk)
{
    ...
    while (pPreviousChunk->bytesRead < pPreviousChunk->length)
    {
        ...
        ThreeDSMaterialInfo* material = pModel->mMaterials.back();
                              ...
                              tChunk *pChunk)
{
    ...
    if (num_to_read == 3)
    {
        m_TempChunk->bytesRead += fread(color, 1, num_to_read, m_FilePointer);
    }
...
#endif
            color[i] = (unsigned char) (buf_union.fbuf[i] * 255);
        }
    }
    else
    {
...
void CLoad3DS::ReadVertexIndices(ThreeDSObject *pObject, tChunk *pPreviousChunk)
{
    ...
    pObject->pFaces = new ThreeDSFace [pObject->numOfFaces];
    memset(pObject->pFaces, 0, sizeof(ThreeDSFace) * pObject->numOfFaces);
...
void CLoad3DS::ReadObjectMaterial(ThreeDSModel *pModel, ThreeDSObject *pObject, tChunk *pPreviousChunk)
{
    ...
    for(size_t i = 0; i < pModel->mMaterials.size(); i++)
    {
        // If the material we just read in matches the current texture name
        if(strcmp(strMaterial, pModel->mMaterials[i]->strName) == 0)
        {
            ...
            {
                pModel->mMaterials[i]->diffuseColor[3] = 100;
                pModel->mMaterials[i]->specularColor[3] = 100;
                pModel->mMaterials[i]->ambientColor[3] = 100;
            }
            ...
            if(strlen(pModel->mMaterials[i]->strFile) > 0) 
            {
